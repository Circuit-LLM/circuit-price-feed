...
async function _rpcFetchPumpSwapPrice(poolAccount, pool, mint) {
  ...
  try {
    ...
    const tokenBalance = readBalance(accounts[0]); // baseVault  → token raw (6 dec)
    const solBalance   = readBalance(accounts[1]); // quoteVault → WSOL lamports (9 dec)

    if (tokenBalance == null || solBalance == null) return null;
    if (tokenBalance === 0   || solBalance === 0)   return null;

    // priceSol = SOL per token = (WSOL_lamports / 1e9) / (token_raw / 1e6)
    const priceSol = (solBalance / 1e9) / (tokenBalance / 1e6);
    ...
    const updatedPool = {
      ...
      baseVault,
      quoteVault,
      _vault0Balance: tokenBalance,
      _vault1Balance: solBalance,
...
async function _velocityGate(mint, result) {
  ...
  // Gate only for non-indexer sources; indexer (Geyser, slot-accurate) is unconditionally trusted.
  if (deviation > VELOCITY_GATE_PCT && result.source !== 'indexer') {
    const jup = await _jupiterPrice(mint).catch(() => null);
    if (jup?.priceSol) {
      const jupDev = Math.abs(jup.priceSol - last) / last;
      if (jupDev <= VELOCITY_GATE_PCT && deviation > VELOCITY_GATE_PCT) {
        // New price is an outlier: it diverges from last known but Jupiter does not.
        // This is a phantom — return Jupiter's price instead.
      ...
      // If Jupiter also moved: legitimate price action — trust the original result.
    }
    // Jupiter unavailable → fail open, return original result.
  }
