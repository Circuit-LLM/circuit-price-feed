...
// Past this age the last-known SOL/USD is considered STALE and getSolPrice() returns null, so consumers
// fall through instead of pricing tokens (and agent trades) off a frozen price when every source is down.
const MAX_PRICE_AGE_MS = Number(process.env.SOL_PRICE_MAX_AGE_MS || 5 * 60_000);
...
function getSolPrice() {
  if (_price == null || _updatedAt == null) return null;
  if (Date.now() - _updatedAt > MAX_PRICE_AGE_MS) return null; // stale — don't trade on a frozen price
  return _price;
}
